...
type Comment struct {
	...
	Subject   string  `boil:"Subject" json:"Subject,omitempty" toml:"Subject" yaml:"Subject,omitempty"`
	Note      float32 `boil:"Note" json:"Note" toml:"Note" yaml:"Note"`
...
var CommentWhere = struct {
	...
	Note      whereHelperfloat32
}{
	ID:        whereHelperint{field: "`comment`.`ID`"},
	ProjectID: whereHelperint{field: "`comment`.`project_id`"},
	UserID:    whereHelperint{field: "`comment`.`user_id`"},
	Subject:   whereHelpernull_String{field: "`comment`.`Subject`"},
	Note:      whereHelperfloat32{field: "`comment`.`Note`"},
}
...
var (
	commentAllColumns            = []string{"ID", "project_id", "user_id", "Subject", "Note"}
	commentColumnsWithoutDefault = []string{"project_id", "user_id", "Subject", "Note"}
...
func (o *Comment) Project(mods ...qm.QueryMod) projectQuery {
	queryMods := []qm.QueryMod{
...
// loaded structs of the objects. This is for an N-1 relationship.
func (commentL) LoadProject(ctx context.Context, e boil.ContextExecutor, singular bool, maybeComment interface{}, mods queries.Applicator) error {
	var slice []*Comment
	var object *Comment
...
// SetProject of the comment to the related item.
// Sets o.R.Project to related.
// Adds o to related.R.Comments.
func (o *Comment) SetProject(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Project) error {
	var err error
	if insert {
		if err = related.Insert(ctx, exec, boil.Infer()); err != nil {
			return errors.Wrap(err, "failed to insert into foreign table")
