...
func testUserRolesExists(t *testing.T) {
	...
	e, err := UserRoleExists(ctx, tx, o.ID, o.UserID)
	if err != nil {
...
func testUserRolesFind(t *testing.T) {
	...
	userRoleFound, err := FindUserRole(ctx, tx, o.ID, o.UserID)
	if err != nil {
		t.Error(err)
	}
...
func testUserRoleToOneSetOpUserUsingUser(t *testing.T) {
	...
	for i, x := range []*User{&b, &c} {
		...
		if exists, err := UserRoleExists(ctx, tx, a.ID, a.UserID); err != nil {
			t.Fatal(err)
		} else if !exists {
...
func testUserRoleToOneSetOpRoleUsingRole(t *testing.T) {
	...
	for i, x := range []*Role{&b, &c} {
		...
		if a.RolesID != x.ID {
			t.Error("foreign key was wrong value", a.RolesID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.RolesID))
		reflect.Indirect(reflect.ValueOf(&a.RolesID)).Set(zero)
		...
		if a.RolesID != x.ID {
			t.Error("foreign key was wrong value", a.RolesID, x.ID)
		}
	}
}
...
var (
	userRoleDBTypes = map[string]string{`ID`: `int`, `RolesID`: `int`, `UserID`: `int`}
	_               = bytes.MinRead
)
...
func testUserRolesUpsert(t *testing.T) {
	...
	if len(mySQLUserRoleUniqueColumns) == 0 {
