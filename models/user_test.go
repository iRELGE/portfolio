...
func testUserToManyUserprofiles(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUserprofiles(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpUserprofiles(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`ID`: `int`, `Email`: `varchar`, `Password`: `varchar`, `Status`: `tinyint`}
	_           = bytes.MinRead
)
