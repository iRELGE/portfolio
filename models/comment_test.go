...
func testCommentToOneProjectUsingProject(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := CommentSlice{&local}
	if err = local.L.LoadProject(ctx, tx, false, (*[]*Comment)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testCommentToOneSetOpProjectUsingProject(t *testing.T) {
	var err error
...
var (
	commentDBTypes = map[string]string{`ID`: `int`, `ProjectID`: `int`, `UserID`: `int`, `Subject`: `varchar`, `Note`: `float`}
	_              = bytes.MinRead
)
