...
func testRoleToManyUserRoles(t *testing.T) {
	...
	for _, v := range check {
		if v.RolesID == b.RolesID {
			bFound = true
		}
		if v.RolesID == c.RolesID {
			cFound = true
		}
	}
...
func testRoleToManyAddOpUserRoles(t *testing.T) {
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if a.ID != first.RolesID {
			t.Error("foreign key was wrong value", a.ID, first.RolesID)
		}
		if a.ID != second.RolesID {
			t.Error("foreign key was wrong value", a.ID, second.RolesID)
		}
...
var (
	roleDBTypes = map[string]string{`ID`: `int`, `Role`: `varchar`}
	_           = bytes.MinRead
)
