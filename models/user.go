...
type User struct {
	...
	Status   bool   `boil:"Status" json:"Status,omitempty" toml:"Status" yaml:"Status,omitempty"`
...
var UserWhere = struct {
	...
	Status:   whereHelpernull_Bool{field: "`user`.`Status`"},
}
...
var (
	userAllColumns            = []string{"ID", "Email", "Password", "Status"}
	userColumnsWithoutDefault = []string{"Email", "Password", "Status"}
...
// Userprofiles retrieves all the userprofile's Userprofiles with an executor.
func (o *User) Userprofiles(mods ...qm.QueryMod) userprofileQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`userprofile`.`user_id`=?", o.ID),
	)

	query := Userprofiles(queryMods...)
	queries.SetFrom(query.Query, "`userprofile`")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"`userprofile`.*"})
	}
...
func (userL) LoadProjects(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`project`),
		qm.WhereIn(`project.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (userL) LoadUserprofiles(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`userprofile`),
		qm.WhereIn(`userprofile.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on userprofile")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for userprofile")
	}
