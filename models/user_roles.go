...
var UserRoleWhere = struct {
	ID      whereHelperint
	RolesID whereHelperint
	UserID  whereHelperint
}{
	ID:      whereHelperint{field: "`user_roles`.`ID`"},
	RolesID: whereHelperint{field: "`user_roles`.`roles_id`"},
	UserID:  whereHelperint{field: "`user_roles`.`user_id`"},
}
...
var (
	userRoleAllColumns            = []string{"ID", "roles_id", "user_id"}
	userRoleColumnsWithoutDefault = []string{"roles_id", "user_id"}
	userRoleColumnsWithDefault    = []string{"ID"}
	userRolePrimaryKeyColumns     = []string{"ID", "user_id"}
)
...
func (userRoleL) LoadRole(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUserRole interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		args = append(args, object.RolesID)
...
func (o *UserRole) SetRole(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Role) error {
	...
	updateQuery := fmt.Sprintf(
		"UPDATE `user_roles` SET %s WHERE %s",
		strmangle.SetParamNames("`", "`", 0, []string{"roles_id"}),
		strmangle.WhereClause("`", "`", 0, userRolePrimaryKeyColumns),
	)
	values := []interface{}{related.ID, o.ID, o.UserID}
...
func FindUserRole(ctx context.Context, exec boil.ContextExecutor, iD int, userID int, selectCols ...string) (*UserRole, error) {
	...
	query := fmt.Sprintf(
		"select %s from `user_roles` where `ID`=? AND `user_id`=?", sel,
	)
...
func (o *UserRole) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for user_roles")
	}
...
var mySQLUserRoleUniqueColumns = []string{}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *UserRole) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	nzDefaults := queries.NonZeroDefaultSet(userRoleColumnsWithDefault, o)
	nzUniques := queries.NonZeroDefaultSet(mySQLUserRoleUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "user_roles", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `user_roles` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for user_roles")
	}
	...
	uniqueMap, err = queries.BindMapping(userRoleType, userRoleMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for user_roles")
	}
...
func (o *UserRole) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM `user_roles` WHERE `ID`=? AND `user_id`=?"
...
func (o *UserRole) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindUserRole(ctx, exec, o.ID, o.UserID)
	if err != nil {
		return err
	}
...
func UserRoleExists(ctx context.Context, exec boil.ContextExecutor, iD int, userID int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `user_roles` where `ID`=? AND `user_id`=? limit 1)"
